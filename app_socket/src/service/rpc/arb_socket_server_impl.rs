...
use crate::protocol::rpc_arb_models::{NodeInfo, NodeType, QueryNodeReq};
use crate::protocol::rpc_arb_socket::arb_socket_service_server::{
    ...
    async fn flush_socket_list(
        ...
        match client.arb_client.list_all_nodes(request).await {
            Ok(data) => {
                ...
                SocketManager::dispatch_mislocated_connections(nodes)
                    .await
                    .unwrap();
                Ok(Response::new(CommonResp {
                    success: true,
