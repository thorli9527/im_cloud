...
pub async fn friend_msg_to_socket(mut body: impl Buf, msg: &OwnedMessage, socket_manager: &Arc<SocketManager>) -> Result<()> {
    ...
    let envelope = Envelope { envelope_id: message_id.clone(), envelope_type: EnvelopeType::ServerToClient as i32, timestamp: now(), payload: Some(FriendEvent(message.clone())) };
