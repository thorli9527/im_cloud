...
impl UserManagerOpt for UserManager {
    ...
    async fn get_user_info(&self, user_id: &UserId) -> Result<Option<ClientEntity>> {
        ...
        let client_opt = client_service.dao.find_one(doc! {  "userId": user_id }).await?;
    ...
    async fn get_user_info_by_name(&self, name: &str) -> Result<Option<ClientEntity>> {
        let client_service = ClientService::get();
        let client_opt = client_service.dao.find_one(doc! {  "username": name }).await?;
    ...
    async fn delete_token(&self, token: &str) -> Result<()> {
        ...
        if let Ok(json) = conn.get::<_, String>(&token_key).await {
            if let Ok(dto) = serde_json::from_str::<ClientTokenDto>(&json) {
                let index_key = format!("token:index:{}:{}", dto.uid, dto.device_type);
    ...
    async fn friend_block(&self, user_id: &UserId, friend_id: &UserId) -> Result<()> {
        ...
        let mongo_friend = if !is_friend {
            // 此处也顺便获取记录用于更新
            friend_service.get_friend_detail(user_id, friend_id).await?
        } else {
            None
        };
