...
// // use redis::{RedisResult, cmd, from_redis_value};
// // === Imports ===
// // 引入所需标准库、第三方库和项目内模块
...
//     Offline { user_id: String , device: DeviceType},
// }
// 
// // === 分片群组结构 ===
// #[derive(Debug, Clone)]
...
//         self.local_group_map.get(group_id).map(|set| set.iter().map(|id| id.clone()).collect()).unwrap_or_default()
//     }
//     // 同步用户信息到 Redis
//     pub async fn sync_user(&self, client: ClientInfo) -> Result<(), AppError> {
//         let user_id = &client.user_id;
//         let mut conn = self.redis_pool.get().await?;
//         let user_info_json = serde_json::to_string(&client)?;
//         let _: () = conn.set(format!("agent:{}:client:{}", client.agent_id,user_id), user_info_json).await?;
//         Ok(())
//     }
//     /// 从 Redis 获取用户信息（反序列化为 ClientInfo）
//     pub async fn get_user_info(&self,agent_id:&str, user_id: &str) -> Result<Option<ClientInfo>, AppError> {
//         let mut conn = self.redis_pool.get().await?;
...
//     }
// 
//     pub async fn sync_group(&self,group:GroupInfo)-> Result<(), AppError> {
//         let group_id = &group.id;
//         let mut conn = self.redis_pool.get().await?;
//         let group_info_json = serde_json::to_string(&group)?;
//         let _: () = conn.set(format!("agent:{}:group:{}",group.agent_id,group_id), group_info_json).await?;
//         Ok(())
//     }
//     /// 从 Redis 获取群组信息（反序列化为 GroupInfo）
//     pub async fn get_group_info(&self,agent_id: &str, group_id: &str) -> Result<Option<GroupInfo>, AppError> {
//         let mut conn = self.redis_pool.get().await?;
//         let key = format!("agent:{}:group:{}",agent_id,group_id);
//         let json: Option<String> = conn.get(&key).await?;
// 
//         match json {
//             Some(data) => {
//                 let group: GroupInfo = serde_json::from_str(&data)?;
//                 Ok(Some(group))
//             }
//             None => Ok(None),
//         }
...
//             let local_has = self.local_group_map.get(group_id).map(|set| set.contains(user_id)).unwrap_or(false);
//             if local_has {
//                 return Ok(local_has);
//             }
//         }
...
//         Ok(())
//     }
//     pub async fn offline(&self, user_id: &str, device: DeviceType) -> Result<(), AppError> {
//         if self.use_local_cache && self.is_responsible(user_id) {
...
// 
//         // 生成下线事件 payload
//         let payload = serde_json::to_string(&UserEvent::Offline {
...
// 
//     /// 分页查询群组内成员列表
//     pub async fn list_group_members(&self, group_id: &str, page: u64, size: u64) -> Result<Vec<String>, AppError> {
//         // 否则从 Redis
//         let mut conn = self.redis_pool.get().await?;
...
// 
//         if start >= all_members.len() { Ok(vec![]) } else { Ok(all_members[start..end].to_vec()) }
//     }
// 
//     /// 分页查询用户加入的群组列表
//     pub async fn list_user_groups(&self, user_id: &str, page: u64, size: u64) -> Result<Vec<String>, AppError> {
//         let mut conn = self.redis_pool.get().await?;
// 
...
// 
//         if start >= group_ids.len() { Ok(vec![]) } else { Ok(group_ids[start..end].to_vec()) }
//     }
// 
//     /// 解散群组（删除成员并广播事件）
...
//             let event = UserEvent::GroupLeave { group_id: group_id.to_string(), user_id: user_id.to_string() };
...
//             if to_delete.len() >= MAX_CLEAN_COUNT {
//                 break;
//             }
//             scanned += 1;
...
//         // let mut cursor = 0u64;
//         // loop {
//         //     let (next_cursor, keys): (u64, Vec<String>) = cmd("SCAN").arg(cursor).arg("MATCH").arg("group:*").arg("COUNT").arg(100).query_async(&mut conn).await?;
//         //     for key in &keys {
...
//         //
//         //             let result: RedisResult<StreamReadReply> = conn.xread_options::<_, _, StreamReadReply>(&[STREAM_KEY], &[">"], &opts).await;
//         //
...
// async fn handle_user_event(event: &UserEvent, shards: &Vec<DashMap<String, ()>>, groups: &ShardedGroupMap, node_id: usize, node_total: usize) {
//     match event {
...
//             groups.entry(group_id.clone()).or_insert_with(DashSet::new).insert(user_id.clone());
//         }
