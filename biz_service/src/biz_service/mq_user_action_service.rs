...
impl UserActionLogService {
    ...
    //禁言
    pub async fn mute(&self, agent_id: &str, uid: &UserId, remark: &str, reason: &str, operator_user: Option<&str>) -> Result<()> {
        let client_service = ClientService::get();
        // 通过 user_id 查找用户，不存在则抛出业务错误
        let mut result = client_service.find_by_user_id(agent_id, uid).await?.ok_or_else(|| AppError::BizError("user.not_found".to_string()))?;
        // 构造操作日志
        let mut action_log = self.build(agent_id, uid, remark, reason, operator_user);
    ...
    pub async fn lock(&self, agent_id: &str, uid: &UserId, remark: &str, reason: &str, operator_user: Option<&str>) -> Result<()> {
        let client_service = ClientService::get();
        // 通过 user_id 查找用户，不存在则抛出业务错误
        let mut result = client_service.find_by_user_id(agent_id, uid).await?.ok_or_else(|| AppError::BizError("user.not_found".to_string()))?;
        // 构造操作日志
    ...
    pub async fn ban(&self, agent_id: &str, user_id: &UserId, remark: &str, reason: &str, operator_user: Option<&str>) -> Result<()> {
        ...
        let client = service.find_by_user_id(agent_id, user_id).await?.ok_or_else(|| AppError::BizError("user.not_found".to_string()))?;
        //修改用户禁言状态
        //redis还需要同步更新
