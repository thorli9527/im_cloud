...
impl GroupInfoService {
    ...
    pub async fn find_by_group_id(&self, group_id: &str) -> Result<GroupInfo, AppError> {
        let result = self.dao.find_one(doc! {"group_id":group_id}).await?;
        match result {
            Some(group) => Ok(group),
    ...
    /// 转让群组（变更 creator_id）
    pub async fn transfer_ownership(&self, group_id: &str, new_owner_id: &str) -> Result<(), AppError> {
