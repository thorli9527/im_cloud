...
impl ShardManagerMqOpt for ShardManager {
    async fn create_group(&self, group_id: &GroupId) -> anyhow::Result<()> {
        ...
        let group_map =
            current.group_shard_map.entry(shard_key.clone()).or_insert_with(DashMap::new);
        ...
        let group_members =
            match GroupMemberService::get().get_all_members_by_group_id(group_id).await {
                Ok(members) => members,
                Err(e) => {
    ...
    async fn destroy_group(&self, msg: &DestroyGroupMsg) -> anyhow::Result<()> {
        ...
        // 4. 打日志记录
        tracing::log::info!("❌ 群组删除成功: group_id={} 分片={}", group_id, shard_index);
        Ok(())
    }
